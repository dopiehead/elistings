...
.open-btn{display: block !important;font-size:; margin-top: 2px;color: black;}
...
.open-btn{display: block !important;font-size:px; margin-top:2px !important;color: black;}
...
<a class="category" href="">Vendor notification</a>
...
$getNotification =mysqli_query($conn,"select * from vendor_notifications where recipient_id ='".htmlspecialchars($_SESSION['business_id'])."' and pending = 0");
...
$.ajax({
type: "POST",
url: "engine/reject-offer.php",
data: {'id':del_id,'sender_id':sender_id},
success: function(response) {
$('.btn-reject').prop('disabled',false);
if (response==1) {
swal({
text:"Offer declined. Buyer will be notified",
icon:"success",
});
}
else{
