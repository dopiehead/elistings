...
//Get the status and the reference of user's payments response.
if(isset($_GET['status']) && isset($_GET['reference']) &&  isset($_GET['id'])){
$user_id =  mysqli_escape_string($conn,$_GET['user_id']);    
$id =  mysqli_escape_string($conn,$_GET['id']);    
$status = mysqli_escape_string($conn,$_GET['status']); 
$reference = mysqli_escape_string($conn,$_GET['reference']); 
if($status =='success'){
$update = mysqli_query($conn,"update item_detail set used = 2 where id = '$id' and user_id = '".htmlspecialchars($_SESSION['business_id'])."'"); 
if($update){echo"<h6 style='color:green;'><b>Payment was successful</b></h6><br>";
    echo"<p>Kindly proceed to your <a href='mylistings.php'>listings</a> page</p>";
}
}
else{echo"<h6><b>Payment was not successful</b></h6><br>";}
}
else{
    echo "<h6>No record found</h6><br>";
}
