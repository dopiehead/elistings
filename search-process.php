...
$condition = "SELECT * from item_detail where sold = 0 and used = 0";
...
<?php
$radius=3;
$pageres = mysqli_query($conn,"select * from item_detail where sold = 0 and used = 0");
...
if ($page>1) {
$previous = $page-1;
echo'<span id="page_num"><a href="search-process.php?search='.$text.'&page='.$previous.'" style="" class="btn-success prev" id="'.$previous.'">&lt;</a></span>';
}
for ($i=1; $i<=$total_num_page; $i++) { 
if(($i >= 1 && $i <= $radius) || ($i > $page - $radius && $i < $page + $radius) || ($i <= $total_num_page && $i > $total_num_page - $radius)) {
if($i == $page) {echo'<span id="page_num"><a href="search-process.php?search='.$text.'&page='.$i.'" class="btn-success active-button" id="'.$i.'">'.$i.'</a></span>';}
  }
...
echo'<span id="page_num"><a  href="search-process.php?search='.$text.'&page='.$i.'" class="btn-success" id="'.$i.'">'.$i.'</a></span>';
}
} 
if ($page<$total_num_page) {
$next = $page+1;
echo'<span id="page_num"><a href="search-process.php?search='.$text.'&page='.$next.'" style="" class="btn-success next" id="'.$next.'">&gt;</a></span>';
}
?>
</tbody></table>
...
<?php
require 'engine/configure.php';
$condition = "SELECT * from item_detail where sold = 0 and product_name like '%$text%' and discount>0 and used = 0";
$data = mysqli_query($conn,$condition);
$datafound = $data->num_rows;
if ($datafound<1) {
echo "<span style='font-size:12px;opacity:0.7;'>No item yet</span>";
...
<?php
require 'engine/configure.php';
$condition = "SELECT * from item_detail where sold = 0 and product_category='$category' and product_price < 10000 and used = 0";
$data = mysqli_query($conn,$condition);
$datafound = $data->num_rows;
if ($datafound<1) {
...
$condition = "SELECT * from item_detail where sold = 0 and used = 0 limit 8";
$data = mysqli_query($conn,$condition);
$datafound = $data->num_rows;
echo "<table><tbody id='mytable' class=''>";
 while($row=mysqli_fetch_array($data))
